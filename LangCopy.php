...
class LanguageController extends Controller
{
    ...
    public function update(Request $request){
        ...
        $data = Language::where('languages_id', $languages_id)->update(['languages_name'=>$languages_name, 'languages_code'=>$languages_code, 'languages_icon'=>$languages_icon,'status'=>$status]);
     ...
     public function trans_edit_key_value(Request $request){
        ...
        if ($lang_key == 'backend'){
            ...
            foreach( $module_list as $key => $module) {
                ...
                $section_list = ModuleSection::where('module_id', $module->module_id)->get();

                foreach($section_list as $k => $group){
                ...
                // foreach ($all_key_group as $key => $group) {
                    ...
                    $all_key_group_value_pair[$module->module_name][$group->section_id]['all_trans_key_val_pair'] = $all_trans_key_val_pair;


                    if($group->section_id == 0)
                        $all_key_group_value_pair[$module->module_name][$group->section_id]['group_name'] = "Default";
                    else
                        $all_key_group_value_pair[$module->module_name][$group->section_id]['group_name'] = !empty($section) ? $section->section_name : '';
